...
> 当前版本：`0.5.0`（[下载](https://github.com/LittleBeeY/memodock/releases/tag/v0.5.0)）
...
- 尚未制作 MSIX 安装包和代码签名。
- 新功能启用前的旧记录没有保存真实的首次保存时间，卡片显示的创建时间以其最后修改时间近似回填；该回填值在迁移时已写入数据，无法事后与真实创建时间区分。
