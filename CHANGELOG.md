...
- 开机自启注册值升级后残留旧路径：启动时校验 Run 注册值指向的程序与当前运行实例一致，不一致自动更正，避免自启动指向已删除的旧发布目录。
...
- 记录卡片显示首次保存时间（绝对时间、精确到分钟，悬停显示完整值）；旧数据以最后修改时间近似回填。
...
- 自包含单文件版改用 `Environment.ProcessPath` 定位开机自启的可执行文件，消除 `Assembly.Location` 恒为空的问题。
